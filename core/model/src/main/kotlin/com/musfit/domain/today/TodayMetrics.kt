...
object MetricResolver {
    ...
    private fun gramMetric(grams: Double, goalGrams: Double): MetricValue = if (goalGrams > 0.0) {
        MetricValue.WithGoal(
    ...
    private fun ratio(value: Double, goal: Double): Float = if (goal <= 0.0) 0f else (value / goal).coerceIn(0.0, 1.0).toFloat()
    ...
    private fun Double.format1(): String = if (this % 1.0 == 0.0) toInt().toString() else String.format(Locale.US, "%.1f", this)
}
