...
class HealthConnectHrvMigration33To34Test {
    ...
    @Test
    fun migration33To34_addsHeartRateVariabilityColumn() {
        createDatabaseFromExportedSchema(version = 33)

        val roomDatabase =
            Room.databaseBuilder(context, MusFitDatabase::class.java, TEST_DATABASE_NAME)
                .addMigrations(DatabaseModule.MIGRATION_33_34, DatabaseModule.MIGRATION_34_35, DatabaseModule.MIGRATION_35_36, DatabaseModule.MIGRATION_36_37, DatabaseModule.MIGRATION_37_38, DatabaseModule.MIGRATION_38_39, DatabaseModule.MIGRATION_39_40, DatabaseModule.MIGRATION_40_41, DatabaseModule.MIGRATION_41_42)
                .build()
        try {
        ...
        try {
            assertTrue(tableHasColumn(migrated, "daily_health_summaries", "hrvRmssdMillis"))
        } finally {
            migrated.close()
        }
    }
    ...
    private fun resolveSchemaFile(version: Int): File {
        ...
        val candidates = listOf(File(relativePath), File("app/$relativePath"), File("../app/$relativePath"), File("../../app/$relativePath"))
    ...
    private companion object {
        const val TEST_DATABASE_NAME = "health-connect-hrv-33-34"
    }
}
